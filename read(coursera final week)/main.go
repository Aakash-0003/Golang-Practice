...
to a slice, and after all lines have been read from the file, your program will have a slice containing one struct for each line
 in the file. After reading all lines from the file, your program should iterate through your slice of structs and print the
...
func main() {
	var filename string
	fmt.Printf("Enter text file name : ")
	fmt.Scan(&filename)
	sliData := make([]Name, 0)
	...
	for scanner.Scan() {
		...
		var name Name
		name.fname, name.lname = data[0], data[1]
		sliData = append(sliData, name)
	}
	file.Close()
	//println(sliData)
	for _, v := range sliData {
